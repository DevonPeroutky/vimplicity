...
function! s:ResizeCommands()
  let velocity_amount = '5'
  let behavior = s:GetResizeBehavior()
  let commands = {
        \  'left'   : ':vertical resize ' . behavior['left']  . velocity_amount,
        \  'right'  : ':vertical resize ' . behavior['right'] . velocity_amount,
        \  'up'     : ':resize ' . behavior['up']   . velocity_amount,
        \  'down'   : ':resize ' . behavior['down'] . velocity_amount,
        \}
  return commands
endfun
...
function! s:ResizeMode(commands)
  ...
  while 1
    ...
    elseif c == s:code_list['finish'] "q
      ...
      echo "Finished!"
      break
    elseif c == s:code_list['enter'] "ESC
      redraw
      echo "Finished!"
      break
